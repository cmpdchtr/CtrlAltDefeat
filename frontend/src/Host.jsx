...
import { Monitor, HelpCircle, Users, ExternalLink, Info, Github, Trophy, Flame } from 'lucide-react';
...
function Host() {
  ...
  useEffect(() => {
    ...
    socket.on('room_created', () => soundManager.play('startup'));
    socket.on('room_update', (roomData) => {
      setRoom(prevRoom => {
        if (prevRoom?.state !== roomData.state) {
          if (roomData.state === 'reveal') soundManager.play('notify');
    ...
    socket.on('timer', (data) => {
      setTimer({ time: data.time, total: data.total });
      if (data.time <= 5 && data.time > 0) soundManager.play('tick');
    });
    return () => { socket.off('room_update'); socket.off('timer'); socket.off('room_created'); };
  }, []);
  ...
  return (
    <div className="w-full h-full relative font-tahoma text-black overflow-hidden select-none">
      <EmojiRain socket={socket} />
      <LanguageSwitcher position="top-right" />
      
      <div className="desktop" onClick={() => setIsStartMenuOpen(false)}>
        {room.state === 'lobby' && (
          <div className="flex items-center justify-center w-full h-full gap-8 p-10">
            <motion.div 
              initial={{ opacity: 0, scale: 0.8, x: -50 }}
              animate={{ opacity: 1, scale: 1, x: 0 }}
              className="window shadow-xl" style={{ width: '400px' }}
            >
              ...
              <div className="window-body text-center p-6 bg-[#ece9d8]">
                <div className="bg-white pt-2 pb-4 mb-[50px] border-2 border-gray-500 shadow-inner">
                  <span className="text-xs font-bold text-gray-500 uppercase tracking-widest">{t('player.roomCode')}</span>
                  <div className="text-[76px] font-mono font-bold text-blue-800">{room.code}</div>
                </div>
                <div className="flex justify-center mb-6"><div className="bg-white p-4 border-2 border-gray-500 shadow-md"><QRCodeSVG value={`${window.location.origin}/?code=${room.code}`} size={200} /></div></div>
                <p className="font-bold text-xl text-blue-900">{t('host.scanToJoin')}</p>
              </div>
            </motion.div>
            ...
            <motion.div 
              initial={{ opacity: 0, scale: 0.8, x: 50 }}
              animate={{ opacity: 1, scale: 1, x: 0 }}
              className="window shadow-xl" style={{ width: '340px', height: '450px' }}
            >
              <div className="title-bar"><div className="title-bar-text">{t('host.connectedPlayersExe')}</div></div>
              <div className="window-body bg-white h-[calc(100%-35px)] overflow-y-auto p-0 border inset shadow-inner"><div className="flex bg-gray-200 border-b p-1 font-bold sticky top-0 z-10">
                ...
                <AnimatePresence>
                  {Object.entries(room.players).map(([sid, p]) => (
                    <motion.div 
                      ...
                      exit={{ opacity: 0, x: 20 }}
                      className="flex items-center border-b p-1 h-[40px] hover:bg-blue-50 transition-colors"
                    >
                      ...
                      <div className="w-16 flex justify-center"><button onClick={() => socket.emit('kick_player', { code: room.code, player_sid: sid })} className="px-2 text-xs bg-red-100 border border-red-300 hover:bg-red-200">{t('host.kick')}</button></div>
                    </motion.div>
                  ))}
                </AnimatePresence>
              </div></motion.div>
        ...
        {['question', 'reveal', 'end'].includes(room.state) && (
          <motion.div 
            ...
            className="window absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2" 
            style={{ width: '95%', height: '90%', maxWidth: '1200px', display: 'flex', flexDirection: 'column' }}
          >
                    ...
                    <table className="w-full text-xl">
                      ...
                      <tbody>{Object.values(room.players).sort((a,b)=>b.score-a.score).map((p, i) => (
                        <tr key={i} className="border-b hover:bg-blue-50 transition-colors">
                          <td className="p-4 flex items-center font-bold text-2xl">
                            {p.avatar !== null && <div className="w-10 h-10 mr-4"><RetroAvatar id={p.avatar} /></div>}
                            {p.name}
                            {i === 0 && <Trophy className="ml-2 text-yellow-500" />}
                          </td>
                          <td className="p-4 text-center font-mono text-3xl font-bold text-blue-700">{p.score}</td>
                          <td className={clsx("p-4 text-center font-bold text-xl", p.status==='alive'?'text-green-600':'text-red-600')}>{p.status.toUpperCase()}</td>
                        </tr>
                      ))}</tbody>
                    </table>
                  </div>
                ...
                <>
                  <div className="flex-shrink-0">
                       ...
                       <div className="flex gap-2">
                         {Object.values(room.players).map((p, i) => (
                           <div key={i} className={clsx(
                             "w-10 h-10 border-2 p-1 transition-all duration-300",
                             p.status === 'dead' ? "grayscale opacity-50 border-red-500" : (p.choice !== null ? "border-green-500 bg-green-50 scale-110 shadow-md" : "border-gray-300")
                           )}>
                             <RetroAvatar id={p.avatar} />
                             {p.combo > 2 && <Flame className="absolute -top-2 -right-2 text-orange-500 animate-bounce" size={16} />}
                           </div>
                         ))}
                       </div>
                       <div className="text-2xl font-bold text-blue-800">
                         {Object.values(room.players).filter(p => p.choice !== null).length} / {Object.values(room.players).filter(p => p.status === 'alive').length} {t('host.answered', 'відповіло')}
                       </div>
                    </div>
                    ...
                    <motion.h2 
                      ...
                      className="text-5xl font-bold my-8 text-center leading-tight text-gray-900 drop-shadow-sm"
                    >
                      {currentQ.question}
                    </motion.h2>
                  </div>
                  <div className={clsx("grid gap-6 flex-grow overflow-y-auto p-4 rounded bg-gray-50 border-2 inset shadow-inner", (currentQ.options?.length > 4) ? "grid-cols-1" : "grid-cols-2")} style={{ alignContent: 'start' }}>
                    {['multiple_choice', 'image_options'].includes(qType) && currentQ.options.map((opt, i) => {
                      ...
                      if (room.state === 'reveal') {
                        style += isCorrect ? "bg-green-500 text-white scale-105 z-10 border-green-700 shadow-xl" : "bg-red-100 opacity-40 grayscale-[0.5]";
                      } else {
                      ...
                      <div className="col-span-2 flex flex-col items-center justify-center h-full gap-8">
                        {room.state==='reveal' ? (
                          <motion.div 
                            ...
                            className="p-10 bg-green-100 border-4 border-green-500 rounded shadow-xl text-5xl font-black text-green-800 animate-bounce"
                          >
          ...
          <AnimatePresence>
            {isStartMenuOpen && (
              <motion.div 
                ...
                className="absolute bottom-[100%] left-0 w-[300px] bg-[#ece9d8] border-2 border-white border-b-gray-600 border-r-gray-600 shadow-2xl z-[10000]"
              >
                <div className="bg-gradient-to-r from-blue-800 to-blue-500 p-4 text-white flex items-center gap-3">
                   <div className="w-10 h-10 bg-white p-1 rounded-md"><Monitor className="text-blue-800" /></div>
                   <div className="font-bold text-lg">CtrlAltDefeat XP</div>
                </div>
                <div className="p-1 flex flex-col">
                  <button className="flex items-center gap-3 p-3 hover:bg-blue-700 hover:text-white text-left transition-colors" onClick={() => window.open('https://github.com/cmpdchtr/CtrlAltDefeat', '_blank')}>
                    <Github size={20} /> <span className="flex-grow">GitHub Repository</span> <ExternalLink size={14} />
                  </button>
                  <button className="flex items-center gap-3 p-3 hover:bg-blue-700 hover:text-white text-left transition-colors">
                    <Info size={20} /> <span className="flex-grow">About Game</span>
                  </button>
                  <div className="h-[1px] bg-gray-400 my-1 mx-2" />
                  <button className="flex items-center gap-3 p-3 hover:bg-blue-700 hover:text-white text-left transition-colors" onClick={() => window.location.reload()}>
                    <HelpCircle size={20} /> <span className="flex-grow">Restart Session</span>
                  </button>
                </div>
                <div className="bg-[#d3d3d3] p-2 flex justify-end">
                   <button className="px-4 py-1" onClick={() => setIsStartMenuOpen(false)}>Log Off</button>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          <button className={clsx("start-btn", isStartMenuOpen && "active")} onClick={(e) => { e.stopPropagation(); setIsStartMenuOpen(!isStartMenuOpen); }}>
            <img src="https://win98icons.alexmeub.com/icons/png/windows_slanted-1.png" alt="" /> {t('host.start')}
          </button>
        </div>
        
        <div className="flex-grow flex items-center px-2">
          {room.state !== 'lobby' && (
            <div className="px-3 py-1 bg-blue-700 text-white rounded border flex items-center text-xs ml-2 shadow-inner">
              <Monitor size={14} className="mr-2"/> {t('host.quizExe')}
            </div>
          )}
        </div>
        <div className="system-tray px-2 flex items-center shadow-inner">
          <Users size={14} className="mr-2" />
          <span className="mr-4">{Object.keys(room.players).length} {t('host.online', 'онлайн')}</span>
          <span className="font-mono bg-black/10 px-2 rounded">{new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
        </div>
      </div>
    </div>
  );
